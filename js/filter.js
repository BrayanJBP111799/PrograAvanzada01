...
function addClass_Show(arrayTravel, classDiv) {
    var i, arr1, arr2;
    arr1 = arrayTravel.className.split(" ");
    arr2 = classDiv.split(" ");
    for (i = 0; i < arr2.length; i++) {
        if (arr1.indexOf(arr2[i]) == -1) {
            arrayTravel.className += " " + arr2[i];
        }
    }
}

//Función para eliminar la clase "show_travel" a los divs que no corresponden a la opción seleccionada.
function removeClass_Show(arrayTravel, classDiv) {
    var i, arr1, arr2;
    arr1 = arrayTravel.className.split(" ");
    arr2 = classDiv.split(" ");
    for (i = 0; i < arr2.length; i++) {
    ...
    arrayTravel.className = arr1.join(" ");
}
